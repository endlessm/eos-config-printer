...
class PrinterDriverOpenPrinting(PrinterDriver):
    ...
    def __init__(self, args):
        ...
        if not args or len(args) < 2:
            raise TypeError("Insufficient number of arguments in %s" % repr(args))

        self._uri = args[0]
        self._fingerprint = args[1]
        self._installedPPDs = []
    ...
    def install(self):
        ...
        filepath = utils.downloadToTemporaryFile(self._uri)
        ...
        extraction_dir = os.path.join(config.TEMPORARY_DIR, 'extract')
        ...
        for path in moved_dirs:
            ...
            (ppd_dirs, ppd_files) = self._searchForPPDFiles(path)
            if ppd_dirs:
                self._createSymlinksForCUPS(ppd_dirs)
            self._installedPPDs = ppd_files

        # Cleanup temporary files before finishing
        shutil.rmtree(config.TEMPORARY_DIR, ignore_errors=True)
    ...
    def _extractDriverPackage(self, driver_path, dest_dir):
        ...
        try:
            ...
            # Wait until the process has actually finished.
    ...
    def _deployDriverDirectories(self, extraction_dir):
        ...
        """
        try:
            dircontents = os.listdir(os.path.join(extraction_dir, 'opt'))
        except OSError as e:
            raise GLib.GError("Error listing contents of directory: %s" % repr(e))
        ...
        for path in dircontents:
            ...
            src = os.path.join(extraction_dir, 'opt', path)
    ...
    def _searchForPPDFiles(self, base_dir):
        ...
        Return a tuple with a list of PPD directories and a list of paths to
        the PPD files if found, or a tuple with two empty lists otherwise.
        """
        result = ([], [])
        dirs_index = 0
        files_index = 1
        for root, dirs, files in os.walk(base_dir):
            for file in files:
                ...
                if file_l.endswith('.ppd') or file_l.endswith('.ppd.gz'):
                    # We only append the directory once.
                    if root not in result[dirs_index]:
                        result[dirs_index].append(root)
                        debugprint("PPD directory found!: %s" % root)

                    ppd_file_path = os.path.join(root, file)
                    result[files_index].append(ppd_file_path)
                    debugprint("PPD file found!: %s" % ppd_file_path)

        debugprint("Found %d PPD directory(s)" % len(result[dirs_index]))
        debugprint("Found %d PPD file(s)" % len(result[files_index]))
        return result
...
class ConfigPrintingService(dbus.service.Object):
    ...
    @dbus.service.method(dbus_interface=CONFIG_PRINTING_IFACE,
                         in_signature='uas', out_signature='as',
    ...
    def _methodIsAuthorized(self, method_name, sender):
        ...
        # This synchronous operation should be very fast, but in any case it's
        # happening inside of a worker thread, so it should not be a problem.
...
class ConfigPrintingClient:
    ...
    def start(self):
        if not self._args or len(self._args) < 3:
            print("Error: Mising printer driver Type, URI and fingerprint")
            return
        ...
        debugprint("Checking permissions through PolKit...")
        try:
            Polkit.Permission.new_sync('com.endlessm.Config.Printing.InstallDriver',
                                       None, None)
        except GLib.GError as e:
            debugprint("And error happened checking for permissions: %s" % e)
            return

        # Just check the permission but don't stop if denied, so that
        # we can test whether the implementation of the D-Bus method
        # blocks from doing anything in this case, as it should.
        bus = dbus.SystemBus()
