...
def downloadToTemporaryFile(uri, dest_dir=config.TEMPORARY_DIR):
    ...
    try:
        ...
        raise GLib.GError("Error downloading file %s: %s" % (uri, e.reason))

    try:
        (tmpfd, filepath) = tempfile.mkstemp(dir=dest_dir)
    except OSError as e:
        raise GLib.GError("Temporary file could not be created: %s" % e)
    ...
    try:
        ...
        raise GLib.GError("File could not be opened: %s" % e)
